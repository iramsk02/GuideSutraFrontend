      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item: any, index: number) => {
            const key = `${nameKey || item.payload?.name || item.dataKey || "value"}`;
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            ...
            return (
              ...
              >
                {formatter && item?.value != null && item.payload ? (
                  formatter(item.value, item.payload.name, item, index, item.payload)
                ) : (
                  <>
                    {itemConfig?.icon ? (
                      <itemConfig.icon />
                    ...
                    <div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
                      <div className="grid gap-1.5">{nestLabel ? tooltipLabel : null}<span className="text-muted-foreground">{itemConfig?.label || item.payload?.name}</span></div>
...
const ChartLegendContent = React.forwardRef<HTMLDivElement, React.ComponentProps<"div"> & CustomLegendProps>(
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: any, key: string) {
  ...
  else if (payloadData && key in payloadData && typeof payloadData[key] === "string") configLabelKey = payloadData[key];
